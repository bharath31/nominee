...
`mode: 'observe'` is the report-only mode: it records policy decisions and
blocks nothing. It must stay impossible to enable by accident — it is refused
under `production: true`, announces itself on startup, and marks every receipt
`enforcement: 'observe'`. See [docs/observe.md](./docs/observe.md).
