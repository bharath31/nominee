...
import { Nominee, allow, deny, ask, MemoryActionStore, MemoryAtomicReceiptStore, PolicyDeniedError, AuthorizationInputChangedError, CapabilityInvalidError, ActionStateError, ActionPendingError } from 'nominee';
...
describe('Security Contract Suite', () => {
  ...
  it('deny-before-execute: denied operations throw before execution', async () => {
    await expect(nominee.authorize({
      tool: 'sensitive.op',
      input: { data: 'test' },
      user: 'u1'
  ...
  it('approval/input binding: input mutation after approval throws AuthorizationInputChangedError', async () => {
    ...
    await nominee.resolveActionApproval(prepareRes.action.id, { decision: 'approved', approver: 'admin', via: 'test' });
    
    const resumed = await nominee.resumeAction(prepareRes.action.id);
    expect(resumed.status).toBe('ready');
    if (resumed.status !== 'ready') return;
    
    await expect(nominee.executeCapability(resumed.capability, { query: 'malicious' }, async () => {})).rejects.toThrow(AuthorizationInputChangedError);
  });
  ...
  it('capability single-use: capability expires after execution', async () => {
    ...
    let ran = false;
    await nominee.executeCapability(capability, { arg: 1 }, async () => { ran = true });
    expect(ran).toBe(true);
    
    await expect(nominee.executeCapability(capability, { arg: 1 }, async () => {})).rejects.toThrow(CapabilityInvalidError);
  });
